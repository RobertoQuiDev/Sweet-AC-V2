...
--                     print("^3[Sweet] ^7 ^1Mauvaise Key ^7: ^1Ta crue que Roberto était con ?^7!")
    ...
    if djsakduiahe789uhqrfyui then
        ...
        AddEventHandler(SwatCC.TrigPrefix ..'driftforscreen', function()
            ...
            for i = 1, #xPlayers, 1 do
                local xPlayer = spyke.GetPlayerFromId(xPlayers[i])
                if xPlayer and xPlayer.getGroup() ~= "user" then
                    TriggerClientEvent('chatMessage', xPlayer.source, "[".. SwatCC.Prefix .."🌊]", {0, 0, 255},'[Possible Moddeur] ' .. GetPlayerName(playerId) .. ' [ID: ' .. playerId ..  '] a déclenché des touches blacklist')
                    Wait(4000)
        ...
        AddEventHandler("tesuntbnktusais", function()
            ...
            DropPlayer(_source, "Louper, Si le problème persiste vidé votre cache de jeu Fivem")
        end)
                    ...
                    function(errorCode, resultDatavpn, resultHeaders)
                        if resultDatavpn == "N" then
                            ...
                            if SwatCS.AntiVPNDiscordLogs then
                                webhooklog("Unauthorized", name, ip, "VPN/PROXY Detecté...", 16515843)
                            end
